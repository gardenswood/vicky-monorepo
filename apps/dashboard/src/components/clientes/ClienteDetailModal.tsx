...
export default function ClienteDetailModal({ telDecoded, onClose }: ClienteDetailModalProps) {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        <div className="bg-white rounded-3xl shadow-2xl p-8 flex items-center justify-center h-64 w-full max-w-4xl">
  ...
  if (!cliente) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        <div className="bg-white rounded-3xl shadow-2xl p-8 text-center w-full max-w-lg border border-slate-100">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-100 animate-in zoom-in-95 duration-200">
        
        {/* Header Fijo */}
        <div className="flex-shrink-0 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white px-6 py-5 flex items-start justify-between gap-4">
          <div className="flex items-center gap-4 flex-1">
            <div className="w-14 h-14 rounded-full bg-brand-100 flex items-center justify-center shadow-inner">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="label text-xs text-slate-400 uppercase tracking-wider mb-1.5">Estado</label>
                    {editing ? (
                      <select
                        className="input"
                        value={draft.estado ?? ''}
                ...
                <div className="mt-5 pt-5 border-t border-slate-50">
                  ...
                  {editing ? (
                    ...
                    <div className="text-sm text-slate-700 bg-slate-50 rounded-xl p-4 border border-slate-100">
                      {cliente.notas || <span className="text-slate-400 italic">Sin notas registradas</span>}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                ...
                {!cliente.pedidosAnteriores || cliente.pedidosAnteriores.length === 0 ? (
                  <div className="p-6 text-center bg-slate-50 rounded-xl border border-slate-100 border-dashed">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {cliente.whatsappLid && (
                    <div>
                      <p className="text-xs text-slate-400 uppercase tracking-wider mb-0.5">WhatsApp LID</p>
                      <p className="text-slate-500 text-xs break-all bg-slate-50 p-1.5 rounded mt-1 border border-slate-100">
                        {cliente.whatsappLid}
                      </p>
                    </div>
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                ...
                {cliente.servicioPendiente ? (
                  <div className="bg-brand-50 rounded-xl p-4 border border-brand-100 shadow-sm">
                    ...
                    <p className="text-xs text-brand-600 mt-1">Cotización pendiente de confirmación</p>
                  </div>
                ) : (
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 border-dashed text-center">
                    <p className="text-slate-400 text-sm italic">Sin servicio activo</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-amber-100 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-16 h-16 bg-amber-50 rounded-bl-full -z-0" />
                <h3 className="font-semibold text-slate-900 mb-3 text-sm flex items-center gap-2 relative z-10">
                  <AlertTriangle className="w-4 h-4 text-amber-600" />
                  Logística leña
                </h3>
                {consultasLena.length > 0 ? (
                  <div className="space-y-3 relative z-10">
                    <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 text-sm text-amber-800 shadow-sm">
                      ...
                      <p className="text-xs mt-2 leading-relaxed">
                        ...
                        Estado: <span className="capitalize">{consultasLena[0].estado.replaceAll('_', ' ')}</span>
                      </p>
                    </div>
                    <Link href="/logistica-zonas" className="btn-secondary w-full justify-center flex items-center gap-1.5 rounded-full bg-white shadow-sm border-slate-200 hover:border-amber-300 hover:text-amber-700">
                      Ver reparto de leña
                    </Link>
                  </div>
                ) : (
                  <div className="space-y-3 relative z-10">
                    <p className="text-slate-400 text-sm text-center italic py-2">Sin consulta logística activa.</p>
                    <button onClick={openConsultaModal} className="btn-secondary w-full flex items-center justify-center gap-1.5 rounded-full border border-dashed border-slate-300 hover:border-brand-300 hover:text-brand-600 bg-white">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                ...
                <p className="text-xs text-slate-600 flex items-center gap-2 bg-slate-50 py-2 px-3 rounded-lg border border-slate-100">
                  ...
                  Audio de bienvenida {cliente.audioIntroEnviado ? 'enviado' : 'pendiente'}
                </p>
              </div>
            </div>
          </div>
